...
export class GuaranteeForm {
  ...
  async submitForm() {
    ...
    console.log("Running form generation");
    ...
    const pdfFields: EmptyMilesPDFFields = {
      ...
      sfcsz: `${this.formBody.value.shipFrom?.city}, ${this.formBody.value.shipFrom?.state} ${this.formBody.value.shipFrom?.zip}`,
      ...
      stcsz: `${this.formBody.value.shipTo?.city}, ${this.formBody.value.shipTo?.state} ${this.formBody.value.shipTo?.zip}`,
      scac: "LSTR",
      stsid: this.formBody.value.shipTo?.cid,
      pro: this.formBody.value.specialInstructions?.freightBill,
      si: `GUARANTEE: ${this.formBody.value.specialInstructions?.emptyMiles} - FREIGHT BILL: ${this.formBody.value.specialInstructions?.freightBill} - OWED: ${this.formBody.value.specialInstructions?.owed} per mile for ${this.formBody.value.specialInstructions?.emptyMiles} Miles - TRUCK #: ${this.formBody.value.specialInstructions?.truckNumber}`,
      pp: "X",
      con1: "GUARANTEE",
      ...
      psy1: "N",
      psn1: "N",
      asi1: `REIMBURSE GUARANTEE (${this.formBody.value.specialInstructions?.emptyMiles})`,
      cd1: `GUARANTEE (${this.formBody.value.specialInstructions?.emptyMiles})`,
      cd2: `*** REIMBURSE ${this.formBody.value.specialInstructions?.emptyMiles} GUARANTEE`,
